...
from solovision.motion.kalman_filters.xyah_kf import KalmanFilterXYAH
...
from solovision.utils.matching import fuse_score, iou_distance, linear_assignment
from solovision.utils.ops import tlwh2xyah, xywh2tlwh, xywh2xyxy, xyxy2xywh
...
class BYTETracker(BaseTracker):
    ...
    def __init__(
        ...
        self.sec_match_thresh = sec_match_thresh
        self.det_thresh = track_thresh
        self.buffer_size = int(frame_rate / 30.0 * track_buffer)
        self.min_frame_hits = int(frame_rate / 30.0 * min_hits)
    ...
    def update(self, dets: np.ndarray, img: np.ndarray = None, embs: np.ndarray = None) -> np.ndarray:
        ...
        for itracked, idet in matches:
            ...
            if track.state == TrackState.Tracked:
                track.update(detections[idet], self.frame_count, self.min_frame_hits)
        ...
        matches, u_track, u_detection_second = linear_assignment(dists, thresh=self.sec_match_thresh)
        for itracked, idet in matches:
            track = r_tracked_stracks[itracked]
            det = detections_second[idet]
            if track.state == TrackState.Tracked:
                track.update(det, self.frame_count, self.min_frame_hits)
                activated_starcks.append(track)
            else:
                track.re_activate(det, self.frame_count, new_id=False)
                refind_stracks.append(track)
        ...
        for itracked, idet in matches:
            unconfirmed[itracked].update(detections[idet], self.frame_count, self.min_frame_hits)
