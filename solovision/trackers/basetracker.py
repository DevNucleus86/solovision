...
class BaseTracker(ABC):
    ...
    def plot_box_on_img(self, img: np.ndarray, box: tuple, conf: float, id: int, thickness: int = 2, fontscale: float = 0.5) -> np.ndarray:
        """
                ...
                for a in active_tracks:
                    if a.history_observations:
                        if len(a.history_observations) > 2:
                            ...
                            img = self.plot_box_on_img(img, box, a.conf, a.cls, a.activation_id, thickness, fontscale)
            ...
            for a in self.active_tracks:
                if a.is_activated:
                    if a.history_observations:
                        if len(a.history_observations) > 2:
                            box = a.history_observations[-1] 
                            img = self.plot_box_on_img(img, box = box, conf = a.conf, id = a.activation_id, thickness = thickness, fontscale = fontscale)
                            if show_trajectories:
                                img = self.plot_trackers_trajectories(img, a.history_observations, a.activation_id)
