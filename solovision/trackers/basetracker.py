        ...
        if self.per_class_active_tracks is not None:
            for k in self.per_class_active_tracks.keys():
                ...
                for a in active_tracks:
                    if a.is_activated and a.history_observations:
                        if len(a.history_observations) > 2:
                            box = a.history_observations[-1]
                            img = self.plot_box_on_img(img, box, a.conf, a.cls, a.temp, thickness, fontscale)
                            if show_trajectories:
                                img = self.plot_trackers_trajectories(img, a.history_observations, a.temp)
        else:
